...
DuelRoom::DuelRoom(RoomManager*roomManager,GameServer*gameServer,unsigned char mode)
    :RoomInterface(roomManager,gameServer),mode(mode),last_winner(-1),user_timeout(nullptr),lflist(3)
{
    createGame();
}
...
bool DuelRoom::isAvailableToPlayer(DuelPlayer* refdp, unsigned char refmode)
{
    ...
    int refScore = refdp->cachedScore;
    ...
    if(refdp->loginStatus == ygo::LoginResult::AUTHENTICATED &&
        getFirstPlayer()->loginStatus == ygo::LoginResult::AUTHENTICATED &&
        !wcscmp(refdp->namew_low,getFirstPlayer()->namew_low))
        return false;


	bool refRanked = refdp->loginStatus == ygo::LoginResult::AUTHENTICATED || refdp->loginStatus == ygo::LoginResult::NOPASSWORD;
	bool firstRanked = getFirstPlayer()->loginStatus == ygo::LoginResult::AUTHENTICATED || getFirstPlayer()->loginStatus == ygo::LoginResult::NOPASSWORD;

	if(mode == MODE_TAG && refRanked !=  firstRanked)
		return false;
...
void DuelRoom::SendBufferToPlayer(DuelPlayer* dp, unsigned char proto, void* buffer, size_t len)
{
	...
	if(dp == &fakeObserver)
	{
		if(proto == STOC_DUEL_START)
			observerReplay.clear();
		char* replayBuffer = (char*) buffer;
		std::vector<char> buf;
		if(len > 0)
			buf.insert(buf.end(),replayBuffer,replayBuffer+len);
		observerReplay.push_back(DuelRoom::FakeObserverMessage(proto,buf));
    ...
    else if(proto==STOC_REPLAY)
    {
		...
		if(last_winner >= 0)
		{
			char* replayBuffer = (char*) buffer;
			lastReplay.clear();
			lastReplay.insert(lastReplay.end(),replayBuffer,replayBuffer+std::min(len,(size_t) 20000));
...
void DuelRoom::ShowPlayerScores()
{
    ...
    for(auto it = players.cbegin(); it!=players.cend(); ++it)
    {
        if(it->first->type == NETPLAYER_TYPE_OBSERVER)
            continue;
        if(it->first->loginStatus != ygo::LoginResult::NOPASSWORD && it->first->loginStatus != ygo::LoginResult::AUTHENTICATED)
        {
        ...
        swprintf(buffer,64,L"%d",it->first->cachedScore);
        wcscat(message,buffer);
    }
...
void DuelRoom::DuelTimer(evutil_socket_t fd, short events, void* arg)
{
	...
	that->duel_mode.StepTimer(fd,events);
   


/** inizio il codice per auto terminare al momento del bisogno **/
    auto last_response = that->duel_mode.getLastResponse();
    int limite =  that->duel_mode.getTimeLimit(last_response);
	
    int trascorso = that->duel_mode.getTimeElapsed();
...
void DuelRoom::createGame()
{
    ...
    ReadyMessagesSent = 0;
    chatReady=true;
	duel_mode->JoinGame(&fakeObserver,nullptr,true);
	duel_mode->ToObserver(&fakeObserver);
}
...
void DuelRoom::InsertPlayer(DuelPlayer* dp)
{
	...
	{
		bool ranked = dp->loginStatus == ygo::LoginResult::AUTHENTICATED || dp->loginStatus == ygo::LoginResult::NOPASSWORD;
		
		for(auto it = players.cbegin(); it!= players.cend(); ++it)
		{
			bool oranked = it->first->loginStatus == ygo::LoginResult::AUTHENTICATED || it->first->loginStatus == ygo::LoginResult::NOPASSWORD;
			if(oranked != ranked && dp->type != NETPLAYER_TYPE_OBSERVER)
				print_trace();
		}
	}
	...
	if(state == PLAYING)
	{
		dp->game = getFirstPlayer()->game;
		
		for(int i = 0; i < observerReplay.size();++i)
		{
			auto & strBuffer = observerReplay[i].second;
			unsigned char proto = observerReplay[i].first;
			void * buffer = nullptr;
			if(strBuffer.size() > 0)
				buffer = &strBuffer[0];
			
			RoomInterface::SendBufferToPlayer(dp,proto,buffer,strBuffer.size());
...
void DuelRoom::LeaveGame(DuelPlayer* dp)
{
    ...
    if(state == PLAYING && duel_mode == dp->game && mode == MODE_TAG && oldtype != NETPLAYER_TYPE_OBSERVER)
    {
    ...
    if(state != ZOMBIE && duel_mode == dp->game && duel_mode)
...
void DuelRoom::Victory(char winner)
{
	...
	if((mode== MODE_SINGLE || mode == MODE_MATCH) && strcmp(_players[NETPLAYER_TYPE_PLAYER1]->ip,"127.0.0.1") &&!strcmp(_players[NETPLAYER_TYPE_PLAYER1]->ip,_players[NETPLAYER_TYPE_PLAYER2]->ip))
        winner = -1;
	...
	if(winner >= 0 && winner <= 2)
	{
		...
		if(  lastReplay.size() > 0)
		{
			...
			char replayName[] = "replay/replayXXXXXX.yrp";
			
			int replayHandle = mkstemps(replayName,4);

			
			if(replayHandle >= 0)
			{
				fchmod(replayHandle,0644);
				write(replayHandle,&lastReplay[0],lastReplay.size());
				close(replayHandle);
				replaySaved = true;
				strncpy(replayCode,&replayName[13],6);
				replayCode[6] = 0;
			}
		...
		if(mode != MODE_HANDICAP)
		{
			nomi.push_back(logger.getPlayerInfo((uintptr_t) _players[NETPLAYER_TYPE_PLAYER1]));
		}
		nomi.push_back(logger.getPlayerInfo((uintptr_t) _players[NETPLAYER_TYPE_PLAYER2]));
		
		
		if(mode == MODE_HANDICAP || mode == MODE_TAG)
		{
			nomi.push_back(logger.getPlayerInfo((uintptr_t) _players[NETPLAYER_TYPE_PLAYER3]));
			nomi.push_back(logger.getPlayerInfo((uintptr_t) _players[NETPLAYER_TYPE_PLAYER4]));
		}
		
		
		
		//Users::getInstance()->saveDuelResult(nomi,winner,replaySaved?std::string(replayCode):"");
		
		resultNomi.clear();
		for(unsigned int i = 0; i < nomi.size();i++)
			resultNomi.push_back(nomi[i]->lpd);
		resultWinner = winner;
		resultReplayCode = replaySaved?std::string(replayCode):"";
	}
...
void DuelRoom::HandleCTOSPacket(DuelPlayer* dp, char* data, unsigned int len)
{
    ...
    case CTOS_RESPONSE:
    {
        ...
        try
        {
            ...
            if(logger.getPlayerInfo((uintptr_t)dp)->hisTurn and duel_mode.getTimeLimit(resp_type) >0 and duel_mode.getTimeLimit(resp_type)<Config::getInstance()->maxTimer)
               duel_mode.setTimeLimit(resp_type,1+ duel_mode.getTimeLimit(resp_type));
        }
