...
struct DuelPlayerStatus
{
    ...
    int modeScelto;
    int spamScelto;
	std::vector<char> lastDeck;
	enum {SHOW_SCORE,SHOW_RANK} scoreRankView;

    DuelPlayerStatus():status(Status::STATS),challenger(nullptr),banlistCompatibili(3),modeScelto(MODE_SINGLE),spamScelto(0),scoreRankView(SHOW_SCORE) {};
};
...
class WaitingRoom:public RoomInterface
{
	...
	enum {QUEUE_SINGLE_UNRANKED_TCG, QUEUE_SINGLE_UNRANKED_OCG, QUEUE_SINGLE_RANKED_TCG, QUEUE_SINGLE_RANKED_OCG,QUEUE_MATCH_RANKED_TCG,QUEUE_MATCH_RANKED_OCG} QueueType;
