...
void WaitingRoom::ShowDuelSettings(DuelPlayer* dp)
{
    ...
    int rank = dp->cachedRank;
    int score = dp->cachedScore;
	wchar_t wmessage[21];
    switch (dp->loginStatus)
    {
		case ygo::LoginResult::NOPASSWORD:
    case ygo::LoginResult::AUTHENTICATED:
		if(player_status[dp].scoreRankView == DuelPlayerStatus::SHOW_SCORE)
			swprintf(wmessage,20, L"Skill:%d",score);
        else
			swprintf(wmessage,20, L"Rank:%d",rank);
		SendNameToPlayer(dp,1,wmessage);
        break;

    case ygo::LoginResult::INVALIDPASSWORD:
        SendNameToPlayer(dp,1,"invalid password");
        break;

    case ygo::LoginResult::INVALIDUSERNAME:
       
        SendNameToPlayer(dp,1,"invalid username");
        break;

    case ygo::LoginResult::UNRANKED:
        SendNameToPlayer(dp,1,"Unranked player!");
        break;
    }
    ...
    {
        if(dp->lflist == 3)
             SendNameToPlayer(dp,3,"Banlist: [AUTO]");
	else if(dp->lflist ==2 )
...
{
    std::vector<DuelRoom *> lista = roomManager->getPlayingRoomsList();
    EnableCrosses(dp);
    SendNameToPlayer(dp,0,"-Available rooms-");
	if(lista.size() > 3)
		lista.erase(lista.begin(), lista.begin() + (lista.size() -3));
    ...
    for(int i=0; i<lista.size() && i<3; i++)
    {
        ...
        sprintf(testo,"%s %d ",tipo,lista[i]->getFirstPlayer()->cachedScore);
...
void WaitingRoom::ButtonKickPressed(DuelPlayer* dp,int pos)
{
    ...
    case DuelPlayerStatus::DUELSETTINGS:
        ...
        {
			if(player_status[dp].scoreRankView == DuelPlayerStatus::SHOW_SCORE)
					player_status[dp].scoreRankView = DuelPlayerStatus::SHOW_RANK;
			else
					player_status[dp].scoreRankView = DuelPlayerStatus::SHOW_SCORE;
           
        }
        if(pos == 2 && !players[dp].isReady)
        {
            if(player_status[dp].modeScelto == MODE_SINGLE)
    ...
    case DuelPlayerStatus::CHOOSESERVER:
        if(pos > player_status[dp].listaStanzeCompatibili.size() )
            break;
        roomManager->insertPlayerAsSpectator(dp,player_status[dp].listaStanzeCompatibili[pos-1]);
        break;
...
bool WaitingRoom::send_challenge_request(DuelPlayer* dp,wchar_t * second,unsigned char mode)
{
		...
		bool ranked1 = (dp->loginStatus == ygo::LoginResult::AUTHENTICATED || dp->loginStatus == ygo::LoginResult::NOPASSWORD);
		bool ranked2 = (dpnemico->loginStatus == ygo::LoginResult::AUTHENTICATED || dpnemico->loginStatus == ygo::LoginResult::NOPASSWORD);
		if(ranked1 != ranked2)
		{
			 SystemChatToPlayer(dp,L"a ranked player cannot challenge a unranked one",true);
            return true;
