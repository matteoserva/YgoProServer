...
struct team_sorter {
  inline bool operator() (const Team& player1, const Team& player2) {
...
void adjust_teams(std::vector<Team> teams){
  ...
  std::sort(teams.begin(), teams.end(), team_sorter());
  unsigned int teams_count = teams.size();
   
  std::vector<Player *> players;
  for(int i_t = 0; i_t< teams_count; i_t++)
	  for(int i_p = 0; i_p < teams[i_t].members.size(); i_p++)
		   players.push_back(teams[i_t].members[i_p]);
		   
 
  unsigned int players_count = players.size();
  ...
  for (i = 0; i < teams_count - 1; ++i) {
    ds.push_back(new Variable());
  }

  std::vector<PriorFactor*> skill;
  for(auto curTeam = teams.begin(); curTeam!= teams.end(); ++curTeam)
	  for(auto curPlayer = curTeam->members.begin(); curPlayer != curTeam->members.end(); ++curPlayer)
	  {
		    Player* pl = *curPlayer;
			Variable* s = ss[skill.size()];
			Gaussian* gaussian = new Gaussian();
  ...
  for(int t_c = 0; t_c < teams.size(); t_c++)
  {
    std::vector<Variable*>* p = new std::vector<Variable*>();
    std::vector<double>* c = new std::vector<double>;
	auto members = teams[t_c].members;
	 for(auto curPlayer = members.begin(); curPlayer != members.end(); ++curPlayer)
	  {
          p->push_back(ps[index]);
          c->push_back((*curPlayer)->weight);
		  index++;
	  }
    perf_to_team.push_back(new SumFactor(ts[t_c], p, c));
  }
  ...
  for (i = 0; i < teams_count - 1; ++i) {
    TruncateFactor* tf;
    if (teams[i].rank == teams[i + 1].rank) {
  ...
  for(std::vector<SumFactor*>::iterator it = perf_to_team.begin(); it != perf_to_team.end(); ++it) {
	  for(int term = 0; term< (*it)->terms->size(); term++)
	  (*it)->update_term(term);
...
double win_probability(Player* player_rating, Player * opponent_rating)
{
         Constants constants;
    	 double delta_mu = player_rating->mu - opponent_rating->mu;
         double denom = sqrt(2 * (constants.BETA * constants.BETA) + player_rating->sigma*player_rating->sigma + opponent_rating->sigma*opponent_rating->sigma);
