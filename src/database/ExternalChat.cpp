...
void ExternalChat::broadcastMessage(protocol::GameServerChat* msg)
{
...
std::list<protocol::GameServerChat> ExternalChat::getPendingMessages()
{

    std::list<protocol::GameServerChat> lista;
    if(Config::getInstance()->noExternalChat)
        return lista;
    ...
    try
    {
        ...
        while(res->next())
        {
            ...
            if(id>last_id)
                last_id = id;
            wchar_t buffer[1024];
            protocol::GameServerChat gsc;
            gsc.chatColor = chatColor;
            gsc.type=protocol::MessageType::CHAT;
			memset(gsc.messaggio,0,sizeof(gsc.messaggio));
...
ExternalChat::ExternalChat(MySqlWrapper *m)
{
	mysqlWrapper = m;
    last_id=0;
    pid = (int)getpid();
