...
bool DuelRoom::isAvailableToPlayer(DuelPlayer* refdp, unsigned char refmode)
{
	...
	bool refRanked = refdp->loginStatus == Users::LoginResult::AUTHENTICATED || refdp->loginStatus == Users::LoginResult::NOPASSWORD;
	bool firstRanked = getFirstPlayer()->loginStatus == Users::LoginResult::AUTHENTICATED || getFirstPlayer()->loginStatus == Users::LoginResult::NOPASSWORD;
...
void DuelRoom::ShowPlayerScores()
{
    ...
    for(auto it = players.cbegin(); it!=players.cend(); ++it)
    {
        if(it->first->type == NETPLAYER_TYPE_OBSERVER)
            continue;
        if(it->first->loginStatus != Users::LoginResult::NOPASSWORD && it->first->loginStatus != Users::LoginResult::AUTHENTICATED)
        {
...
void DuelRoom::InsertPlayer(DuelPlayer* dp)
{
	...
	{
		bool ranked = dp->loginStatus == Users::AUTHENTICATED || dp->loginStatus == Users::NOPASSWORD;
		
		for(auto it = players.cbegin(); it!= players.cend(); ++it)
		{
			bool oranked = it->first->loginStatus == Users::AUTHENTICATED || it->first->loginStatus == Users::NOPASSWORD;
			if(oranked != ranked)
				print_trace();
		}
	}
...
bool DuelRoom::InsertPlayerAsSpectator(DuelPlayer* dp)
{
	if(state != PLAYING)
		return false;
    //it inserts forcefully the player into the server
